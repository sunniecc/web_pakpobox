        ...
        $scope.onSelectPage = function(page){
            ...
            Api.getExpress(data, function (json) {
                ...
                list && list.map(function (e,i) {
                    var arr = [];
                    arr[0] = e.expressNumber+$scope.IfAddOverFlag(e.overdueFlag);
                    arr[1] = e.storeUser?e.storeUser.name:'no';
                    arr[2] = e.takeUserPhoneNumber?e.takeUserPhoneNumber:'no';
                    arr[3] = e.storeTime?moment(e.storeTime).format('YYYY-MM-DD HH:mm'):'no';
                    arr[4] = e.takeTime?moment(e.takeTime).format('YYYY-MM-DD HH:mm'):'no';
                    ...
                    arr[8] = '<div class="dropdown">' +
                                ...
                                '<ul class="dropdown-menu" aria-labelledby="dLabel">' +
                                    ...
                                    '<li><button class="btn  btn-sm send-select clickResetStatus" '+($scope.ifAdmitResetStatus(e.status)?'':'disabled')+'  data-id="' + e.id + '">'+ $scope.LResetStatus +'</button></li>'+

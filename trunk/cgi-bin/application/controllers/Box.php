...
class BoxController extends Yaf_Controller_Abstract {
    ...
    //查询柜子名称
    //index/box/check/exist
    //Method : GET
    public function queryBoxNameAction(){
		if (null == ($authInfo = UtilsModel::authRight())){
			//没有权限
			return ;
		}
		...
		if (!UtilsModel::checkParams($paramArr,array("name"))){
			//参数错误
		...
		$ret = UtilsModel::get_by_curl("box/check/nameExist",
			UtilsModel::getQueryStr($paramArr),
			array("Content-Type:application/json","userToken:".$token));
		UtilsModel::dealHttpCode($ret);
    ...
    public function rebuildAction(){
		...
		//向api sever注册请求
		$ret = UtilsModel::post_by_curl("box/rebuild",
			$postData,array("Content-Type:application/json","userToken:".$token));
    ...
    //修改柜子状态
    public function updateBoxStatusAction()
    {
    	//先检验登陆态
		if (null == ($authInfo = UtilsModel::authRight())){
			//没有权限
		...
		//参数检验
		if (!UtilsModel::checkParams($boxInfo,array("id","status"))){
			//参数错误
			$ret["statusCode"] = CODE_PARAM_ERROR;
			$ret["msg"] = "params error";
			echo UtilsModel::getUrlJson($ret);
		...
		//向api sever注册请求
		$ret = UtilsModel::post_by_curl("box/status/update",
			$postData,array("Content-Type:application/json","userToken:".$token));

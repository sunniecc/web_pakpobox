...
(function(angular) {
    ...
    myModule.controller('main', ['$scope', '$templateCache', '$filter', function($scope, $templateCache, $filter) {
        ...
        var statusMap = {
            ...
            EXCEPTION:$scope.LPakpoboxException
        }
            ...
            Api.queryBoxInfo(data, function (json) {
                ...
                list && list.map(function (e,i) {
                    ...
                    arr[4] = '<div class="dropdown">' +
                        '<button class="btn '+btnColorMap[e.status]+' btn-sm dropdown-toggle" data-id='+e.id+' id="dropdownMenu" data-toggle="dropdown" value='+e.status+'>'+
                        statusMap[e.status]+
                        '</button>' +
                        '</div>';
        ...
        //禁用启用箱子
        $('body').on('click', '#dropdownMenu', function () {
            var id = $(this).data('id')
             var status =$(this).val();
             if(status=='EXCEPTION'){
                Alert.show($scope.LExceptionBoxState);
                return;
             }
             $('#changeBoxState').modal('show');
             ...
             Api.updateBoxStatus(data, function (json) {
                if(json.statusCode==0)
                {
                    $('#changeBoxState').modal('hide');  
                    if(status=='DISABLE'){
                        Alert.show($scope.LPakpoboxRunSuccess+$scope.LSuccess,'success');
                    }else{
                        Alert.show($scope.LPakpoboxStopSuccess+$scope.LSuccess,'success');
                    }
                     setTimeout(function() {

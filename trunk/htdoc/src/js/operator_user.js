...
(function(angular) {
    'use strict';
    angular.module('operatorUser', ['ngSanitize'])
        .controller('main', ['$scope', '$templateCache', '$filter', function($scope, $templateCache, $filter) {
            ...
            if(id){
                ...
                Api.companyInfo(data, function (json) {
                    if(json.statusCode == 0 && json.result){
                        ...
                        $scope.Mail = json.result.contactEmail;
                        $scope.$apply();
                    }
                })
            }
            ...
            $scope.submit = function () {
                ...
                contactPhoneNumberList = phoneNumber.split(",");
                contactEmaillist = Mail.split(",");
                for (var i = 0; i <contactPhoneNumberList.length; i++) {
                    contactPhoneNumberList[i] = contactPhoneNumberList[i].trim();
                };
                for (var i = 0; i <contactEmaillist.length; i++) {
                    contactEmaillist[i] = contactEmaillist[i].trim();
                };
                var data = {
                    // contactName: $scope.loginName,
                    contactPhoneNumber: contactPhoneNumberList,

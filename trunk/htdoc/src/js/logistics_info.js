    ...
    myModule.controller('main', ['$scope', '$templateCache', '$filter', function($scope, $templateCache, $filter) {
        ...
        Api.userInfo(function(json){
            if(json.statusCode != 0){
                ...
                if($scope.userrole=="LOGISTICS_COMPANY_USER"){
                    $scope.info.title=$scope.LLogisticsInfo + ' <a href="logistics_edit.html?id=' + id + '"><small>>></small></a>';
                    $scope.info.secondTitle=$scope.LLinkmanInfo  + ' <a href="logistics_user.html?id=' + id + '"><small>>></small></a>';
                }
                else{
                    $scope.info.title=$scope.LLogisticsInfo + ' <a href="logistics_edit.html?id=' + id + '"><small>>>' + $scope.LToEdit + '</small></a>';
                    $scope.info.secondTitle=$scope.LLinkmanInfo  + ' <a href="logistics_user.html?id=' + id + '"><small>>>' + $scope.LToEdit + '</small></a>';
                }
                $scope.$apply();
            }
        })
        ...
        Api.companyInfo({companyType: 'LOGISTICS_COMPANY',companyId:id}, function (json) {
            ...
            if(result.contactPhoneNumber){
                ...
                // $scope.info.contactPhoneNumber = "<p class='info-value' ng-bind="item.value"></p>"
            }
